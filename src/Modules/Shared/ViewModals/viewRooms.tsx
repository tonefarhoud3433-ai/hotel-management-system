...
export default function ViewRooms({ open, onClose, facility }: ModalProps) {
  const roomData: RoomDetailData | null = facility?.room ? facility.room : facility;
  ...
  return (
    ...
    >
      {/* Header */}
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', px: 2.5, pt: 1.5, pb: 0.5 }}>
        <Typography 
          variant="subtitle2" 
          sx={{ 
            color: '#1976d2', 
      ...
      <DialogContent sx={{ px: 2.5, py: 0.5, overflowY: 'hidden' }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1.5 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            <Box sx={{ bgcolor: '#eff6ff9e', p: 1, borderRadius: 3, display: 'flex', alignItems: 'center' }}>
        ...
        <Grid container spacing={1.5} sx={{ mb: 1.5 }}>
          {/* Base Price Card */}
          <Grid item xs={4}>
            <Card variant="outlined" sx={{ p: 1, borderRadius: 3, border: '1px solid #f3f4f6', bgcolor: '#fbfbfb', textAlign: 'center' }}>
              <Stack sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 0.3 }}>
                <AttachMoneyIcon sx={{ color: '#059669', fontSize: '1.1rem' }} />
                <Typography variant="caption" display="block" sx={{ fontWeight: 600, color: '#6b7280', fontSize: '0.65rem' }}>
                  Base Price
                </Typography>
              </Stack>
              <Typography variant="body2" sx={{ color: '#111827', fontWeight: 700, mt: 0.3 }}>
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
              {facilities.map((fac: any, index: number) => (
                <Chip 
                  key={fac._id || index} 
                  label={fac.name || fac} 
                  icon={<StarIcon style={{ color: '#d97706', fontSize: '0.8rem' }} />}
                  size="small"
                  variant="outlined" 
                  sx={{ 
                    borderRadius: '6px', 
        ...
        <Box sx={{ mb: 1.5, p: 1, bgcolor: '#f8fafc', borderRadius: 2.5, border: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: 1 }}>
          <KeyIcon sx={{ color: '#94a3b8', fontSize: '1.1rem' }} />
          <Box>
            <Typography variant="caption" display="block" sx={{ fontWeight: 700, color: '#64748b', fontSize: '0.65rem', textTransform: 'uppercase' }}>
              Room Reference Key ID
            </Typography>
            <Typography variant="body2" sx={{ color: '#334155', fontWeight: 600, fontFamily: 'monospace', fontSize: '0.75rem' }}>
              {roomData?._id || "N/A"}
            </Typography>
          </Box>
        </Box>
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <Box>
              <Typography variant="caption" display="block" sx={{ color: '#64748b', fontWeight: 500, fontSize: '0.65rem' }}>Created At</Typography>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <Box>
              <Typography variant="caption" display="block" sx={{ color: '#64748b', fontWeight: 500, fontSize: '0.65rem' }}>Last Modified</Typography>
              <Typography variant="caption" sx={{ fontWeight: 600, color: '#1e293b', fontSize: '0.75rem' }}>
        ...
        >
          <Typography variant="body2" sx={{ fontSize: '0.75rem', lineHeight: 1.4 }}>
            Active configuration logs state that this asset entry was created by Administrator: <strong style={{ color: '#1976d2', fontWeight: 700 }}>{creatorName}</strong>.
          </Typography>
        </Box>
